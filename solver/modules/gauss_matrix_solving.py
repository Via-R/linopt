...
def gauss_solve(a):
    eps = 1e-16
    basis_change = numpy.eye(len(a), dtype=int)
    c = numpy.array(a)
    a = numpy.array(a)
    len1 = len(a[:, 0])
    len2 = len(a[0, :])
    ...
    for g in range(len1):
        ...
        if my != g:
            ...
            chronos = copy.deepcopy(basis_change[g])
            basis_change[g] = copy.deepcopy(basis_change[my])
            basis_change[my] = copy.deepcopy(chronos)
